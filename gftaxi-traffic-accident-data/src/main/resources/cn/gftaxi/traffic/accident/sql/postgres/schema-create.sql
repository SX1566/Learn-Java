...
create table gf_accident_situation (
  ...
  register_checked_attachments jsonb
);
...
create or replace view bs_car as
  select id, stage, plate_type, plate_no, factory_type, operate_date, company, motorcade_name, charger, driver, code, manage_no
  from dblink(
  ...
  as t(
    id integer, stage integer, plate_type varchar(255), plate_no varchar(255), factory_type varchar(255),
...
comment on column bs_car.stage         is '状态：-2:新购,-1:草稿,0-在案,1-注销';
...
create or replace view bs_carman as
  select id, uid, stage, type, name, sex, origin, id_card_no, initial_license_date, model, work_date, service_cert_no, phone
  from dblink(
    'dbname=bcsystem host=192.168.0.7 user=reader password=reader',
  ...
  as t(
    id integer, uid varchar(36), stage integer, type integer, name varchar(255), sex integer, origin varchar(255),
...
comment on column bs_carman.stage               is '状态：-1:草稿,0-启用中,1-已禁用,2-已删除';
