...
open class AccidentSubListBaseInfo : IdEntity, DynamicBean() {
  ...
  companion object {
    ...
    fun copy(source: AccidentSubListBaseInfo, target: AccidentSubListBaseInfo) {
      target.apply {
        source.id?.let { this.id = source.id }
        source.sn?.let { this.sn = source.sn }
        source.name?.let { this.name = source.name }
        source.type?.let { this.type = source.type }
        source.followType?.let { this.followType = source.followType }
        source.updatedTime?.let { this.updatedTime = source.updatedTime }
      }
    }
  }
}