...
/**
 * Test [AccidentStatServiceImpl.statReportQuarterlySummary].
 *
 * @author RJ
 */
...
class StatQuarterlySummaryMethodImplTest @Autowired constructor(
  ...
  fun success() {
    ...
    `when`(accidentStatDao.statReportQuarterlySummary(from, to)).thenReturn(Flux.fromIterable(expected))
    `when`(securityService.verifyHasAnyRole(*ROLES_REPORT_READ)).thenReturn(Mono.empty())

    // invoke
    val actual = accidentStatService.statReportQuarterlySummary(from, to)
    ...
    verify(accidentStatDao).statReportQuarterlySummary(from, to)
  }
  ...
  fun `Failed by PermissionDenied`() {
    ...
    // invoke and verify
    StepVerifier.create(accidentStatService.statReportQuarterlySummary(from, to))
      .expectError(PermissionDeniedException::class.java)
      .verify()
    verify(securityService).verifyHasAnyRole(*ROLES_REPORT_READ)
    verify(accidentStatDao, times(0)).statReportQuarterlySummary(from, to)
  }
