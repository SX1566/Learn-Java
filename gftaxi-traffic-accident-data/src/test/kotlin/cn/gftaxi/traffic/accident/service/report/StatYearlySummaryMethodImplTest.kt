...
/**
 * Test [AccidentStatServiceImpl.statReportYearlySummary].
 *
 * @author RJ
 */
...
class StatYearlySummaryMethodImplTest @Autowired constructor(
  ...
  fun success() {
    ...
    `when`(accidentStatDao.statReportYearlySummary(from, to)).thenReturn(Flux.fromIterable(expected))
    `when`(securityService.verifyHasAnyRole(*ROLES_REPORT_READ)).thenReturn(Mono.empty())

    // invoke
    val actual = accidentStatService.statReportYearlySummary(from, to)
    ...
    verify(accidentStatDao).statReportYearlySummary(from, to)
  }
  ...
  fun `Failed by PermissionDenied`() {
    ...
    // invoke and verify
    StepVerifier.create(accidentStatService.statReportYearlySummary(from, to))
      .expectError(PermissionDeniedException::class.java)
      .verify()
    verify(securityService).verifyHasAnyRole(*ROLES_REPORT_READ)
    verify(accidentStatDao, times(0)).statReportYearlySummary(from, to)
  }
