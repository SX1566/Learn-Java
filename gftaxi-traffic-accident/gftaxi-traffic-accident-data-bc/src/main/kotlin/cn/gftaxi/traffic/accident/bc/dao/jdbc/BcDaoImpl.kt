...
class BcDaoImpl @Autowired constructor(
  ...
  override fun findMotorcade(includeDisabledStatus: Boolean): Flux<MotorcadeDto> {
    val statuses = arrayListOf<Short>().apply {
      add(MotorcadeStatus.Enabled.value())
      if (includeDisabledStatus) add(MotorcadeStatus.Disabled.value())
    }
    val sql = """
    select m.id, m.name, row_number() over() as sn, m.status_ as status, a.id as branch_id,
      a.name as branch_name, m.principal_id as captain_id, m.principal_name as captain_name
    from bs_motorcade m
    inner join bc_identity_actor a on a.id = m.unit_id
    where m.status_ in (:statuses)
    order by a.order_, m.code
    """.trimIndent()
    return jdbc.queryForList(sql, mapOf("statuses" to statuses)).let {
      val motorcadeList = it.map {
        MotorcadeDto(id = "${it["id"]!!}".toInt(), name = it["name"]!!.toString(), sn = "${it["sn"]!!}".toInt(),
          status = MotorcadeStatus.values().find { v -> v.value() == "${it["status"]}".toShort() }!!,
          branchId = "${it["branch_id"]!!}".toInt(), branchName = it["branch_name"]!!.toString(),
          captainId = "${it["captain_id"]!!}".toInt(), captainName = it["captain_name"]!!.toString())
      }
      if (it.isEmpty()) Flux.empty() else Flux.fromIterable(motorcadeList)
    }
  }
