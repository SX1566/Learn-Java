...
/**
 * Test [AccidentStatServiceImpl.statRegisterQuarterlySummary].
 *
 * @author RJ
 * @author zh
 */
...
class StatQuarterlySummaryMethodImplTest @Autowired constructor(
  ...
  fun success() {
    ...
    val expected = listOf(
      dto.copy(scope = "2017Q4"),
      dto.copy(scope = "2017Q3"),
      dto.copy(scope = "2017Q2"),
      dto.copy(scope = "2017Q1")
    )
    `when`(accidentStatDao.statRegisterQuarterlySummary(from, to)).thenReturn(Flux.fromIterable(expected))
    `when`(securityService.verifyHasAnyRole(*ROLES_REGISTER_READ)).thenReturn(Mono.empty())

    // invoke
    val actual = accidentStatService.statRegisterQuarterlySummary(from, to)
    ...
    verify(accidentStatDao).statRegisterQuarterlySummary(from, to)
  }
  ...
  fun `Failed by PermissionDenied`() {
    ...
    // invoke and verify
    StepVerifier.create(accidentStatService.statRegisterQuarterlySummary(from, to))
      .expectError(PermissionDeniedException::class.java)
      .verify()
    verify(securityService).verifyHasAnyRole(*ROLES_REGISTER_READ)
    verify(accidentStatDao, times(0)).statRegisterQuarterlySummary(from, to)
  }
