...
interface AccidentStatService {
   ...
   * 事故报告按月汇总统计。
   *
   * 1. 如果用户没有 [事故报告查阅相关角色][ROLES_REPORT_READ]，
   *    则返回 [PermissionDeniedException] 类型的 [Flux.error]。
   * 2. 如果[统计的开始年月][from] 和 [结束年月][to] 跨度大于两年或开始年月大于结束年月，
   *    则返回 [IllegalArgumentException] 类型的 [Flux.error]。
   * 3. 返回结果按时间逆序排序。
   ...
   * @param[to]   统计的结束年月，默认为当年的 12 月
   */
  fun statReportMonthlySummary(from: YearMonth = YearMonth.of(Year.now().value, 1),
                               to: YearMonth = YearMonth.of(Year.now().value, 12))
   ...
   * @param[to]   统计的结束年份，默认为当年
   */
  fun statReportYearlySummary(from: Year = Year.now().minusYears(1),
                              to: Year = Year.now())
    : Flux<AccidentReportDto4StatSummary>
   ...
   * @param[to]   统计的结束年份，默认为当年
   */
  fun statReportQuarterlySummary(from: Year = Year.now(), to: Year = Year.now())
    : Flux<AccidentReportDto4StatSummary>
}