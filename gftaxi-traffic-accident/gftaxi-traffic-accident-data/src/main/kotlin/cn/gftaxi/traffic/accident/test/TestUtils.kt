...
val OffsetDateTime.yearQuarter: String
  get():String {
    return "${this.year}Q${this.get(IsoFields.QUARTER_OF_YEAR)}"
  }
...
object TestUtils {
  ...
  /** 在指定区间随机 [start, end) 生成一个长整型数字 */
  fun randomLong(start: Long = 0, end: Long = 100) = ThreadLocalRandom.current().nextLong(start, end)

  /** 在指定区间随机 [start, end) 生成一个时间 */
  fun randomOffsetDateTime(start: OffsetDateTime, end: OffsetDateTime) = start.plusNanos(
    randomLong(0, end.toInstant().toEpochMilli() - start.toInstant().toEpochMilli()) * 1000000
  )
  ...
  /** 构建新的案件上报数据 */
  fun randomAccidentDraftDto4FormSubmit(
    happenTime: OffsetDateTime = OffsetDateTime.now().truncatedTo(ChronoUnit.MINUTES).minusDays(10)
  ): AccidentDraftDto4FormSubmit {
    return AccidentDraftDto4FormSubmit().also {
      it.happenTime = happenTime
      it.carPlate = randomString("car")
      it.driverName = randomString("driver")
      it.location = randomString("location")
      it.hitForm = randomString("hitForm")
      it.hitType = randomString("hitType")
      it.describe = randomString("describe")
      it.source = "BC"
      it.authorName = randomString("authorName")
      it.authorId = randomString("authorId")
    }
  ...
  /** 构建新的报告表单数据 */
  fun randomAccidentReportDto4Form(
    id: Int? = nextId("Case"),
    happenTime: OffsetDateTime = OffsetDateTime.now().truncatedTo(ChronoUnit.MINUTES).minusDays(10),
    code: String = nextCaseCode(happenTime),
    draftTime: OffsetDateTime = happenTime.plusHours(12 - 1),
    ...
    registerTime: OffsetDateTime = happenTime.plusHours(24 - 1),
    overdueRegister: Boolean? = isOverdue(happenTime, registerTime, 24 * 60 * 60),
    registerStatus: AuditStatus = AuditStatus.Approved,
    reportTime: OffsetDateTime? = happenTime.plusHours(48 - 1),
    overdueReport: Boolean? = isOverdue(happenTime, registerTime, 48 * 60 * 60),
    reportStatus: AuditStatus = AuditStatus.ToSubmit
  ): AccidentReportDto4Form {
    return AccidentReportDto4Form().also {
      it.id = id
      it.code = code
      it.happenTime = happenTime
      ...
      it.reportTime = reportTime
      it.overdueReport = overdueReport
      it.reportStatus = reportStatus
  ...
  /** 构建新的报告汇总统计数据 */
  fun randomAccidentReportDto4StatSummary(
    scope: String = randomString("RM")
  ): AccidentReportDto4StatSummary {
    val checked = randomInt(0, 100)
    val checking = randomInt(0, 100)
    val reporting = randomInt(0, 100)
    val closed = randomInt(0, 100)
    val total = reporting + checking + checked + closed
    return AccidentReportDto4StatSummary(
      scope = scope,
      checked = checked,
      checking = checking,
      reporting = reporting,
      closed = closed,
      total = total,
      overdueReport = randomInt(0, total)
    )
  }

  /** 构建新的维修报告视图数据行*/
  fun randomAccidentRepairDto4View(
    id: Int? = nextId("case"),
    repairStatus: MaintainStatus = MaintainStatus.ToSubmit

  ): RepairDto4View{
    return RepairDto4View(
      id = id ,
