...
interface AccidentDao {
   ...
   *    - 设置 [AccidentSituation.draftTime] = OffsetDateTime.now()。
   *    - 自动计算 [是否逾期报案][AccidentSituation.overdueDraft]。
   *    - 自动从 BC 系统获取相应的车辆和司机信息：motorcadeName、carX、driverX、historyX。
   ...
   */
  fun createCase(caseData: AccidentDraftDto4FormSubmit): Mono<Pair<AccidentCase, AccidentSituation>>
