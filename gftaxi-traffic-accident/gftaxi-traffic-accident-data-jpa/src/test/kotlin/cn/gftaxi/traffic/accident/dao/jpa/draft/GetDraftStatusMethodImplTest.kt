...
/**
 * Test [AccidentDaoImpl.getDraftStatus].
 *
 * @author RJ
 * @author zh
 */
...
class GetDraftStatusMethodImplTest @Autowired constructor(
  ...
  @Test
  fun `Get empty`() {
    StepVerifier.create(dao.getDraftStatus(randomInt())).verifyComplete()
  }
  ...
  fun `Get it`() {
    ...
    // invoke and verify
    StepVerifier.create(dao.getDraftStatus(accidentCase.id!!))
      .expectNext(accidentSituation.draftStatus!!).verifyComplete()
  }
}