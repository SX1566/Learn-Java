...

{
  ...
  rows: [
    {code, status, carPlate, driverName, happenTime, createTime, location, 
     hitForm, hitType, overdueCreate, source, authorName, authorId},
    ...
  ]
}
```
...
rows/createTime | true    | 报案时间，格式为 yyyy-MM-dd HH:mm
rows/location   | true    | 事发地点
rows/hitForm    | true    | 事故形态，如 "车辆间事故"
rows/hitType    | true    | 碰撞类型，如 "追尾碰撞"
rows/overdueCreate | true | 是否逾期报案，true-预期、false-正常
...

{
  ...
  hitForm, hitType, createTime, overdueCreate, location, describe, source
}
```
...
createTime | true    | 报案时间，格式为 yyyy-MM-dd HH:mm
overdueCreate | true | 是否逾期报案，true-预期、false-正常
location   | true    | 事发地点
describe   | false   | 简要描述
source     | true    | 报案来源
...
[
  {scope, total, checked, checking, drafting, overdueCreate, overdueRegister},
  ...
]
```
...
[
  {id, code, carPlate, driverName, driverType, happenTime, location, motorcadeName, hitForm, hitType, 
   authorName, authorId, createTime, overdueCreate, registerTime, overdueRegister, submitTime},
  ...
]
```
...
authorId        | true    | 接案人账号
createTime      | true    | 报案时间，格式为 yyyy-MM-dd HH:mm
overdueCreate   | true    | 是否逾期报案，true 或 false
...
```
{
  ...
  // 基本信息：事故
  id, code, status, createTime, happenTime, locationLevel1, locationLevel2, locationLevel3, location, 
  ...
  cars: [
    {
      name, type, model, towCount, towMoney, repairType, repairMoney, 
...
> AccidentRegisterInfo 的属性比较多，提交时只需要提交设置了值的属性，值没有设置的属性不要提交。
...
createTime                 | String  | 报案时间，格式为 yyyy-MM-dd HH:mm
...
location                   | String  | 事发地点的县级下面的详细地点
...
{passed, comment, attachmentId, attachmentName}
