...
```
{
  ...
  // 基本信息：事故
  id, code, status, draftTime, happenTime, locationLevel1, locationLevel2, locationLevel3, location, 
...
Name   | Require | Description
-------|---------|-------------
status | false   | 状态，`ToSubmit`-待报告、`ToCheck`-待审核、`Rejected`-审核不通过、`Approved`-审核通过，没有则不限定，多个状态间用英文逗号连接
...
| id                      | Int     | 事故 ID
| code                    | String  | 事故编号，格式为 yyyyMMdd_nn
| reportStatus            | String  | 报告状态：ToSubmit、ToCheck、Rejected、Approved
| motorcadeName           | String  | 事发车队，如 "一分一队"
...
如果用户没有相应的权限或指定的状态条件不在允许的范围内，响应返回：
