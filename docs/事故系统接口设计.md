...
```
GET {context-path}/accident-register/form/category
```
...
{
  motorcades, locationLevel1List, locationLevel2List, locationLevel3List, dealDepartments, dealWays, insuranceCompanies, 
  accidentForms, accidentLevels, hitForms, hitTypes, weathers, lightList, drivingDirections, roadTypes, roadStructures,
  roadStates, partyTypes, carTypes, followTypes, repairTypes, carDamageStates, duties, peopleDamageStates
}
```

Name                    | Description
------------------------|-------------------------------------
motorcades              | 事发车队分类信息，如"一份一队"等
locationLevel1List      | 事发地点的省级分类信息，如"广东省"等
locationLevel2List      | 事发地点的地级分类信息，如"广州市"等
locationLevel3List      | 事发地点的县级分类信息，如"荔湾区"等
dealDepartments         | 处理部门分类信息，如"中保芳村"等
dealWays                | 处理方式分类信息，如"快处快赔"等
insuranceCompanies      | 保险公司分类信息，如"中保芳村"等
accidentForms           | 事故性质分类信息，如"财产损失事故"等
accidentLevels          | 事故等级分类信息，如"财产1级"等
hitForms                | 事故形态分类信息，如"车辆与人事故"等
hitTypes                | 碰撞类型分类信息，如"追尾碰撞"等
weathers                | 天气分类信息，如"晴"等
lightList               | 光线条件分类信息，如"良好"等
drivingDirections       | 行驶方向分类信息，如"由南向北"等
roadTypes               | 道路类型分类信息，如"城市一般道路"等
roadStructures          | 路面状况分类信息，如"良好"等
roadStates              | 路表情况分类信息，如"干燥"等
partyTypes              | 当事人、车辆分类信息，如"自车"、"三者"等
carTypes                | 当事车辆的车型分类信息，如"出租车"、"小轿车"等
followTypes             | 跟进方式分类信息，如"电话"等
repairTypes             | 维修分类信息，如"厂修"、"外修"等
carDamageStates         | 车辆受损情况分类信息，如"车头受损"、"车尾受损"等
duties                  | 事故责任分类信息，如"全责"、"无责"等
peopleDamageStates      | 人员伤亡分类信息，如"无"、"伤" 或 "亡"
