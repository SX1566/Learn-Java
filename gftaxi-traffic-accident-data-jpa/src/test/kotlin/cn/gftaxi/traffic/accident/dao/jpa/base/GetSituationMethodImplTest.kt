...
/**
 * Test [AccidentDaoImpl.getSituation].
 *
 * @author RJ
 */
@SpringJUnitConfig(ModuleConfiguration::class)
@DataJpaTest
class GetSituationMethodImplTest @Autowired constructor(
  ...
  @Test
  fun `Get empty`() {
    StepVerifier.create(dao.getSituation(randomInt())).verifyComplete()
  }
  ...
  fun `Get it`() {
    ...
    // invoke and verify
    StepVerifier.create(dao.getSituation(accidentSituation.id!!))
      .consumeNextWith { assertEquals(accidentSituation.id, it.id) }
      .verifyComplete()
  }
}